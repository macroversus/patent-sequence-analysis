...
#### 时间分片机制（突破 API 单次 100 条限制）

**问题**：Catalyst+ API 的分页功能不起作用，`pageNum=1/2/3/4/5` 都返回相同的第 1 页数据，单次查询最多只能获取 100 条专利。

**解决方案**：自动时间分片
- 当检测到查询结果 ≥100 条时，自动触发时间分片机制
- 将大时间范围（如 1950-2026）切分为多个小时间片（每片 2-6 个月）
- 逐个查询每个时间片，确保每片 <100 条，避免截断
- 自动去重合并所有时间片的结果
...
**默认起始时间**：`1950-01-01`（生物技术专利的实际起始时间，避免查询无意义的 1900-1950 年代）

**示例**：
```
搜索 cutinase (2015-2026)
→ 第 1-5 页都返回相同 100 条（API 限制）
→ 自动触发时间分片：切分为 24 个片段
→ 逐片查询去重：最终获取 410 条专利
```
...
│     ⚠️ API 分页不起作用（pageNum 无效，只返回第 1 页）        │
│     💡 时间分片机制：结果 ≥100 条时自动切分时间范围查询       │
│              │                                              │
│  ② 批量查详情 → 拿到每个专利的完整正文                       │
